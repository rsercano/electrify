...
App.prototype.bundle_meteor = function (/* server_url, */ done) {
    ...
    spawn('meteor' + (this.$.env.os.is_windows ? '.bat' : ''), [
        ...
        self.log.info('trying to copy tunnel-ssh if exist');

        var tunnel_ssh_path = join(programs_server_dir, 'npm', 'node_modules', 'tunnel-ssh');
        if (fs.existsSync(tunnel_ssh_path)) {
            self.log.info('found tunnel-ssh, copying to meteor/modules-runtime/node-modules');
            shell.cp('-r', tunnel_ssh_path, join(programs_server_dir, 'npm', 'node_modules', 'meteor', 'modules-runtime', 'node_modules'));
        }
